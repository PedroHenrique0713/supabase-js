    ...
    <div className="flex flex-col h-[calc(100vh-8rem)] max-w-4xl mx-auto">
      ...
      <div className="flex gap-4 flex-1 min-h-0">
          ...
          <div className="flex-1 overflow-y-auto space-y-3">
            {messages.length === 0 ? (
              ...
              messages.map((msg) => (
                <div
                  key={msg.id}
                  className={cn('flex flex-col', msg.username === username && 'items-end')}
                >
        ...
        <Card className="w-48 p-4 hidden md:block">
          <h3 className="font-medium mb-3">Online ({onlineUsers.length})</h3>
          <ul className="space-y-2">
            {onlineUsers.map((user, i) => (
              <li key={`${user.username}-${i}`} className="flex items-center gap-2 text-sm">
